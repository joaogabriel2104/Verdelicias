...
function verificarDados() {
    ...
    const litSenha = document.getElementById("litSenha");
    ...
    if (nome.value != "" && nome.value != null)
    {
        ...
        if (user.value != "" && user.value != null)
        {
            ...
            if (email.value.includes("@") && email.value != null)
            {
                litEmail.innerText = "";
                if (parseInt(telefone.value) > 11111111111)
                {
                    ...
                    {
                        litSenha.classList.add("vermelho");
                        litSenha.innerText = "DIGITE UMA SENHA VÁLIDA";
                        window.scrollTo({
                            top: 0,
                            behavior: 'smooth',
                            duration: 2000
...
function cadastrarUsuario() {
    return new Promise((resolve, reject) => {
        if (verificarDados())
        {
            ...
            const telefone = document.querySelector("#txtTelefone");
            const senha = document.querySelector("#txtSenha");
            const senhaConfirmar = document.querySelector("#txtSenhaConfirmar");
            ...
            if (email.value == emailConfirmar.value) {
                if (senha.value == senhaConfirmar.value) {
                    ...
                    if (!isNaN(dataNascimentoDate.getTime())) {
                        ...
                        fetch('lib/inserirUsuario.aspx', opcoesRequisicao)
                    ...
                    litErroSenha.classList.add("vermelho");
                    litErroSenha.innerText = "SENHA INCORRETA";
                    window.scrollTo({
                        top: 0,
                        behavior: 'smooth',
                        duration: 2000
